...
template <typename T>
/*A helper function that literally just sets a number if its too big or too
   low.*/
...
extern "C" {
/*Will take a float and pow it by an int. powf can do the same but is slower
 * then this.*/
...
/*
    ...
    Pass in a value from -127 to 127 and it will perform the math to feel
   slightly better

    Source:
   https://github.com/purduesigbots/forkner-public/blob/62e1328b7902715035357622d81e4a35cb15ff2f/src/libforkner/drive.cpp#L363
*/
...
float linear_slew(float current, float target,
    ...
    bool reversing = (current * target) < 0;
    float limit = reversing ? decelRate : accelRate;
